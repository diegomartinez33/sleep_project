...
def createDataset_PP(datasetNum=0, windowSize=30, rawImages=False, delOuts='No', 
                  timesOut=0, 
                  jobPathPrint=os.path.join(homeDic,'Tesis','results'),
                  savePath=os.path.join(homeDic,'Tesis','Data','Per_Patient')):
    """ Function that creates the defined database Per Patient
    ...
    dataDir_Orginized = os.path.join(homeDic,'Tesis','Data','Organized')
    #dataSets = ['SC', 'ST', 'SC_ST']
    dataSets = ['SC', 'ST']
    ...
    # cont = 0
    dataPath = os.path.join(dataDir_Orginized,dataType) 
    for patient in os.listdir(dataPath):
        ...
        for file in os.listdir(patientPath):
            if file.endswith(".npz"):
                ...
                line = 'Database: {} Patient: {} ' 
                line += 'Record: {}\n'
                next_line = line.format(dataType, patient, file)
                with open(job_print_dir,'a') as f:
                    f.write(next_line)
                    print(next_line)
                    f.close()
    
        #Create dictionary per patient
        patientDict = {}
        if delOuts == 'Yes' and timesOut:
            patientDict['data'] = delOutilers(Data,timesOut)
        else: 
            patientDict['data'] = Data
            
        patientDict['data2'] = Data2
        patientDict['Targets'] = Targets
        
        #Get raw images
        if rawImages:
            image_line_ini = 'Start Creation of images for patient: {}\n'
            image_line_ini = image_line_ini.format(patient)
            image_line_end = 'Finished Creation of images for patient: {}\n'
            image_line_end = image_line_end.format(patient)
            ...
            # Get images from data in patientDict
            imgData = expD.getSigImages(patientDict,ostype=1)
            patientDictImg = {}
            patientDictImg['images'] = imgData
            patientDictImg['Targets'] = Targets
            
            #Add image dictionary to big dictionary of data
            FinalDict = patientDictImg
            print('Patient dictionary created: ', patient)
            ...
            #Add amplitude dictionary to big dictionary of data
            FinalDict = patientDict
            print('Patient dictionary created: ', patient)
    

        dataBaseName = 'dataBaseDict_{}_{}_{}_{}.pkl'
        dataBasefile = dataBaseName.format(dataType,imgType,windowSize,timesOut)
        dataSavePath = os.path.join(savePath, dataType, patient, dataBasefile)

        if not os.path.isdir(os.path.join(savePath, dataType, patient)):
            os.makedirs(os.path.join(savePath, dataType, patient))
        
        pickle.dump(FinalDict, open(dataSavePath, 'wb'), 
                    pickle.HIGHEST_PROTOCOL)
        print('Saved dataBase: ', dataBaseName, ' for patient: ', patient, '\n')
