...
# /home/gaperezs/disco2/nodules_pyTorch/checkpoints/model_epoch_3.pth
...
# Functions
# --------------------------------------------------------------------------------------------------------------
ACCpath = osp.join(TesisPath, 'results/trainResults_try')
modelsPath = osp.join(TesisPath, 'results/trainModels_try')
...
def trainDOEexp(balancedPath=data_base_path,Ei=1):
    ...
    for file in os.listdir(balancedPath):
        if file == "dataBaseDict_SC_amp_30_3.pkl":
            print(file)
            saveComb = "try_SC_30_3" # Folder to save results and models
            for i in range(1,4):
...
# TODO test for every combination
...
def loadDataBase(data_path=data_base_path,data_file=data_base_file,trainFold=1):
    ...
    print('labels: ', label_train.shape)
    time.sleep(10)
...
def train(epoch,saveFolder='',trainFold=1):
    ...
    # TODO crear el osp.join(ACCpath, saveFolder) si no existe
