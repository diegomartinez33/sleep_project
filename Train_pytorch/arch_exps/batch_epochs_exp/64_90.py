...
ACCpath = osp.join(TesisPath, 'results','kernel_exps','trainResults', batch_size + '_' + num_epochs)
modelsPath = osp.join(TesisPath, 'results/kernel_exps/trainModels', batch_size + '_' + num_epochs)
...
netsfolder = osp.join(codesTrainfolder, 'nets', 'kernels_exp')
...
from square_9xS_net import Net_30s
from square_9xS_net import Net_10s
from square_9xS_net import Net_30s_ST
from square_9xS_net import Net_10s_ST

# --------------------------------------------------------------------------------------------------------------
# Parsers
# --------------------------------------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='PyTorch MNIST Example')
parser.add_argument('--batch-size', type=int, default=int(batch_size), metavar='N',
                    help='input batch size for training (default: 64)')
parser.add_argument('--test-batch-size', type=int, default=1000, metavar='N',
                    help='input batch size for testing (default: 1000)')
parser.add_argument('--epochs', type=int, default=int(num_epochs), metavar='N',
...
# Functions
# --------------------------------------------------------------------------------------------------------------
someList = ['dataBaseDict_SC_amp_10_3.pkl', 'dataBaseDict_SC_amp_30_3.pkl']

def train_kernel_exp(balancedPath=data_base_path,Ei=1):
    """Function to train all combinations for """
    print('Start training\n')
    for file in os.listdir(balancedPath):
        if file.endswith(".pkl") and file in someList:
            print(file)
