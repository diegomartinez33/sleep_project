...
ACCpath = osp.join(TesisPath, 'results/kernel_exps/trainResults_raw', kernel_shape, 'transposed')
modelsPath = osp.join(TesisPath, 'results/kernel_exps/trainModels_raw', kernel_shape, 'transposed')
...
from square_480x9_transpose_net import Net_raw
...
# Functions
# --------------------------------------------------------------------------------------------------------------
someList = ['dataBaseDict_SC_raw_10_3.pkl', 'dataBaseDict_SC_raw_30_3.pkl']
...
def trainDOEexp(balancedPath=data_base_path,Ei=1):
    ...
    for file in os.listdir(balancedPath):
        if file.endswith(".pkl") and file in someList:
            ...
            for i in range(1,4):
                loaders = loadDataBase(args, balancedPath, file, trainFold=i, raw=True)
                windowSize = int(file[-8:-6])
                dataType = file[-15:-13]

                # Define model
                defineModel(Net=Net_raw)
